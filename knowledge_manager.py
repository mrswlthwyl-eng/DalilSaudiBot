...
class KnowledgeManager:
    ...
    def search(self, user_text: str) -> dict:
        ...
        result = self._search_sections(user_text, university_data)
        if result:
    ...
    def _search_sections(self, query: str, data: dict) -> Optional[dict]:
        """
    ...
    def _search_section(
        ...
        if isinstance(section_data, list):
            ...
            if query in section_data.lower():
                return {
                    "found": True,
                    "section": section_key,
    ...
    def _search_list(
        ...
        """Search a list of items for a match."""
        for item in items:
            if isinstance(item, dict):
                # Check name, title, description, category
                searchable = " ".join(
            ...
            if isinstance(value, str) and value:
                if query in key.lower() or query in value.lower():
                    return {
                        ...
                        "answer": value if value.startswith("http") else value,
...
# ============================================================
# Singleton instance (optional, for easy import)
# ============================================================
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 60)
    test_queries = [
        "جامعة الملك فيصل البلاك بورد",
        "KFU كلية الهندسة",
        "الملك فيصل التقرير النهائي",
        "جامعة الملك سعود القبول",
    ]
